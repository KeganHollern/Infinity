...
void PrintStringsFromArray(Framework::Array* pArray)
{
	if (!pArray)
	{
		Infinity::Logging::Error("Array NULL!");
		return;
	}
	for (int i = 0; i < pArray->Size; i++)
	{
		...
		{
			Infinity::Logging::Print("%s", value);
		}
	}
	...
	for (int i = 0; i < pArray->Size; i++)
	{
		...
		if (value)
			if(value->pType && value->pType->name)
				Infinity::Logging::Print("Entry: %p | %s", value, value->pType->name);
	}
}
...
void PrintMapInfo(Framework::Hashmap* pMap)
{
	if (!pMap)
	{
		Infinity::Logging::Error("Map NULL!");
		return;
	}
	Infinity::Logging::Print("Map: %p", pMap);
	for (int i = 0; i < pMap->Size; i++)
	{
		...
		if (key && value && value->pType && value->pType->name)
		{
			...
			if(test == value)
				Infinity::Logging::Print("Map: %s, %s (successfully called Get)", key, value->pType->name);
			else
				Infinity::Logging::Print("Map: %s, %s (failed to call Get)", key, value->pType->name);
		}
	}
}
...
void ArrayUsagePluginInit()
{
	...
	Infinity::Logging::Print("ArrayUsage Plugin Loaded.");
}