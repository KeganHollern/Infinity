...
void PrintToConsole(char* value)
{
	Logging::Println(value);
}
...
void*** LoadSpecialModule(Framework::ScriptModule* pModule)
{
	if (!dayzLoadScript)
	{
		Logging::Errorln("Failed to find dayz load script function");
		return NULL;
	}
	...
	Logging::Println("Loading Special Module!");
...
void FindPatternPluginInit()
{
	...
	Infinity::Enfusion::Enscript::RegisterFunction("PrintToDayZConsole", &PrintToConsole);
	...
	if (Infinity::Enfusion::RegisterKeyPath(pTemp, "sptmp", true))//should allow read & write from "$dab:<file_name>"
		Logging::Println("Registered Key Path!");
	else
		Logging::Errorln("Failed to register key path!");

	Infinity::Logging::Println("FindPattern Plugin Loaded.");
	*/
}
